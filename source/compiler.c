...
static void emitLoop(int loopStart) {
    ...
    int offs = currentChunk()->count - loopStart + 2;
    if (offs > UINT16_MAX) error("Body of loop too large");

    emitByte((offs >> 8) & 0xff);
    emitByte(offs & 0xff);
}
...
static void patchJump(int offs) {
    int jump = currentChunk()->count - offs - 2;

    if (jump > UINT16_MAX) {
        error("Too much code to jump over.");
    }

    currentChunk()->code[offs] = (jump >> 8) & 0xff;
    currentChunk()->code[offs+1] = jump & 0xff;
}
...
static void declareVariable() {
    ...
    for (int i = current->localCount - 1; i>=0; i--){
        ...
        if (identifiersEqual(name, &local->name)){
            error("Already a variable with this name declared in current scope.");
        }
    }
...
static void classDeclaration() {
    ...
    uint8_t nameConst = identifierConstant(&parser.previous);
    declareVariable();
...
static void forStatement() {
    ...
    consume(TOKEN_SEMICOLON, "Expect '(' after 'for'.");
